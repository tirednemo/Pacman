...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        if(len(foodDistances)!=0):
            nearest_food_distance = min(foodDistances)
            score+= 1/(nearest_food_distance + 1)
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        # util.raiseNotDefined()
        return self.value(gameState, 0, self.depth, alpha = -1e9, beta = 1e9)[1]
    ...
    def minimizer(self, gameState, agentIndex, depth, alpha, beta):
        ...
        for action in actionList:
            successorGameState = gameState.generateSuccessor(agentIndex, action)
            successorScore = self.value(successorGameState, nextAgent, nextDepth, alpha, beta)[0]
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def value(self, gameState, agentIndex, depth):
        if gameState.isLose() or gameState.isWin() or depth == 0:
            ...
            return self.expectimizer(gameState, agentIndex, depth)
        
    def expectimizer(self, gameState, agentIndex, depth):
        nextAgent = (agentIndex + 1) % gameState.getNumAgents()
        if nextAgent == 0:
            nextDepth = depth - 1
        else:
        ...
        currentScore, currentAction = 0, Directions.STOP
        actionList = gameState.getLegalActions(agentIndex)
...
def betterEvaluationFunction(currentGameState):
    ...
    foodDistances = [manhattanDistance(food, currentPos) for food in currentFood.asList()]
    if(len(foodDistances)!=0):
        nearest_food_distance = min(foodDistances)
        score+= 1/(nearest_food_distance + 1)
    ...
    for ghostState in currentGhostStates:
        ghostPos = ghostState.getPosition()
        ghostDistance = manhattanDistance(ghostPos, currentPos)
        if ghostDistance < 1:
            if ghostState.scaredTimer > 0:
                score += 1 / (ghostDistance + 1) 
            elif ghostState.scaredTimer == 0:
